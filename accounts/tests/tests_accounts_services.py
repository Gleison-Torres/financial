...
from accounts.services import send_activation_email, send_password_reset_email


class TestEmailServicesActivation(TestCase):
    """
...
class TestEmailServicesPasswordReset(TestCase):
    ...
    def test_send_password_reset_email(self):
        ...
        send_password_reset_email(self.request, self.user)
        self.assertEqual(len(mail.outbox), 1)

        email = mail.outbox[0]

        self.assertEqual(email.subject, 'Redefinição de senha')
        self.assertEqual(email.to, ['gleison@email.com'])
        self.assertEqual(email.content_subtype, 'html')
        self.assertIn(self.user.username, email.body)
